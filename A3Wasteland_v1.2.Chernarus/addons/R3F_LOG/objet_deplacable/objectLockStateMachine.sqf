...
switch (_lockState) do
{
	...
	case 1: // UNLOCK
	{
		...
		_istheirs = if (_object getVariable ["ownerUID", ""] == getPlayerUID player) then { 1 } else { 0 }; // Allow owner to unlock quickly	
		if(_istheirs == 0) exitWith{player globalChat "Sorry, on Cuntee`s server you can only unlock objects that are yours! Give each player a base wall for a secure base!";};
