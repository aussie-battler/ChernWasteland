	...
	if (!isNil "_itemEntry" && {{_x == _marker} count allMapMarkers > 0}) then
	{
		...
		if (_player getVariable ["cmoney", 0] >= _itemPrice) then
		{
			...
			switch (true) do
			{
				...
				case (_object isKindOf "Land_Pod_Heli_Transport_04_box_F"):
				{
					_object setAmmoCargo 25;
				};
				
				case (_object isKindOf "Land_Pod_Heli_Transport_04_bench_F"):
				{
					_object setAmmoCargo 25;
				};
				
				case (_object isKindOf "Land_Pod_Heli_Transport_04_medevac_F"):
				{
					_object setAmmoCargo 25;
				};
