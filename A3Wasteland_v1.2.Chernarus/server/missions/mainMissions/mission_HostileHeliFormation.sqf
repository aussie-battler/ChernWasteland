...
_setupObjects =
{
	...
	{
		(_heliChoices select 0) set [0, "CUP_B_AH64D_USA"];
		(_heliChoices select 1) set [0, "CUP_B_AH1Z_AT"];
		(_heliChoices select 2) set [0, "CUP_B_Mi24_D_CDF"];
	};
	...
	_createVehicle =
	{
		...
		switch (true) do
		{
			...
			case (_type isKindOf "CUP_B_AH64D_USA" || _type isKindOf "CUP_B_AH1Z_AT" || _type isKindOf "CUP_B_Mi24_D_CDF"):
			{
	...
	_missionHintText = format ["A formation of armed helicopters containing a <t color='%3'>%1</t> and two <t color='%3'>%2</t> are patrolling Chernarus. Destroy them and recover their cargo!", _vehicleName, _vehicleName2, mainMissionColor];
